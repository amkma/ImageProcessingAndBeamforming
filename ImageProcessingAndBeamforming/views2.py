...
class BeamformingView(View):
        ...
        try:
            ...
            if success:
                ...
                return JsonResponse({
                    ...
                    'current_index': simulator.current_array_index,
                    'total_arrays': len(simulator.arrays),
                    'message': f'Array {array_id} removed successfully'
                })
            else:
                return JsonResponse({'error': 'Array not found'}, status=404)
        ...
        try:
            ...
            if new_array:
                ...
                return JsonResponse({
                    ...
                    'array': new_array.to_dict(),
                    'current_index': simulator.current_array_index,
                    'scenario_name': simulator.scenarios.get(scenario_id, {}).get('name', ''),
                    'metrics': metrics,
                    'message': f'Loaded {scenario_id} scenario successfully'
                })
            else:
    ...
    def handle_get_status(self, request):
        ...
        try:
            ...
            status_data = {
                ...
                'session': {
                    'has_quick_saves': 'quick_saves' in request.session,
    ...
    def get_simulator(self, request):
        ...
        if 'simulator' not in request.session:
            ...
            # Load existing simulator
            simulator = BeamformingSimulator()
            try:
                simulator.import_configuration(request.session['simulator'])
            except:
                # If loading fails, create new simulator
                simulator = BeamformingSimulator()
                simulator.create_array()
                request.session['simulator'] = simulator.export_configuration()
                request.session.modified = True
...
class APIDocumentationView(View):
    ...
    def get(self, request):
        ...
        api_endpoints = {
            'POST /api/beamforming/': {
                'description': 'Main endpoint for beamforming operations',
                'actions': {
                    'update_array': 'Update array configuration',
            ...
            'GET /api/beamforming/export/': {
                'description': 'Export current configuration as JSON'
            },
            'POST /api/beamforming/import/': {
